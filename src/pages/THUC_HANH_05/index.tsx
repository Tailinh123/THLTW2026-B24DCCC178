...
import {
  loadClubs, saveClubs, loadMemberships, saveMemberships,
  loadHistory, saveHistory, genId,
} from './types';
...
const THUC_HANH_05: React.FC = () => {
  ...
  const addHistory = (thaoTac: string, doiTuong: string, ghiChu = '') => {
    const item: LichSuThaoTac = { id: genId(), thoiGian: new Date().toLocaleString('vi-VN'), thaoTac, doiTuong, ghiChu };
  ...
  const handleAddMembership = (data: Omit<DonDangKy, 'id'>) => {
    ...
    setMemberships(next); saveMemberships(next);
  ...
  const handleApprove = (ids: string[]) => {
    const next = memberships.map(m => ids.includes(m.id) ? { ...m, trangThai: 'Approved' as const } : m);
    setMemberships(next); saveMemberships(next);
  ...
  const handleReject = (ids: string[], lyDo: string) => {
    const next = memberships.map(m => ids.includes(m.id) ? { ...m, trangThai: 'Rejected' as const, lyDoTuChoi: lyDo } : m);
    setMemberships(next); saveMemberships(next);
    const names = memberships.filter(m => ids.includes(m.id)).map(m => m.hoTen).join(', ');
  ...
  const handleTransferClub = (ids: string[], newClubId: string) => {
    const next = memberships.map(m => ids.includes(m.id) ? { ...m, clubId: newClubId } : m);
    setMemberships(next); saveMemberships(next);
  ...
  return (
    <Layout className="clb-root">
      {ctx}
      <Sider width={240} className="clb-sidebar" style={{ position: 'sticky', top: 0, height: '100vh', overflow: 'auto' }}>
        <div className="clb-sidebar-brand">
          <div className="clb-sidebar-logo">CL</div>
          <div>
            <div className="clb-sidebar-title">Quản lý CLB</div>
            <div className="clb-sidebar-subtitle">CLUB MANAGER</div>
          </div>
        </div>
        <Menu mode="inline" selectedKeys={[page]} onClick={({ key }) => setPage(key as PageKey)}
          items={[
      ...
      <Layout>
        <Header className="clb-header" style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <Text className="clb-header-title">{PAGE_TITLE[page]}</Text>
          <div className="clb-breadcrumb">Trang chủ / <span style={{ color: '#0f172a', fontWeight: 600 }}>{PAGE_TITLE[page]}</span></div>
        </Header>
        <Content className="clb-content">
          {page === 'dashboard' && <DashboardPage clubs={clubs} memberships={memberships} />}
          {page === 'clubs' && <ClubListPage clubs={clubs} memberships={memberships} onAdd={handleAddClub} onEdit={handleEditClub} onDelete={handleDeleteClub} />}
          {page === 'memberships' && <MembershipPage memberships={memberships} clubs={clubs} history={history} onAdd={handleAddMembership} onEdit={handleEditMembership} onDelete={handleDeleteMembership} onApprove={handleApprove} onReject={handleReject} />}
          {page === 'members' && <MembersPage memberships={memberships} clubs={clubs} onTransfer={handleTransferClub} />}
        </Content>
      </Layout>
    </Layout>
  );
