...
interface Props {
  open: boolean;
  editRecord: DonDangKy | null;
  clubs: CauLacBo[];
  onCancel: () => void;
  onSubmit: (data: Omit<DonDangKy, 'id'>) => void;
}
...
const MembershipFormModal: React.FC<Props> = ({ open, editRecord, clubs, onCancel, onSubmit }) => {
  ...
  const handleOk = () => {
    form.validateFields().then((values) => {
      onSubmit({
        ...values,
        trangThai: editRecord?.trangThai ?? 'Pending',
        lyDoTuChoi: editRecord?.lyDoTuChoi ?? '',
  ...
  return (
    <Modal
      visible={open}
      title={editRecord ? 'Chỉnh sửa đơn đăng ký' : 'Thêm đơn đăng ký mới'}
      onCancel={onCancel}
      onOk={handleOk}
      ...
      <Form form={form} layout="vertical" style={{ marginTop: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
          <Form.Item name="soTruong" label="Sở trường">
            <Input placeholder="Lập trình, Âm nhạc..." />
          </Form.Item>
        ...
        <Form.Item name="lyDo" label="Lý do đăng ký" rules={[{ required: true, message: 'Nhập lý do' }]}>
          <TextArea rows={3} placeholder="Nêu lý do muốn tham gia câu lạc bộ..." />
        </Form.Item>
