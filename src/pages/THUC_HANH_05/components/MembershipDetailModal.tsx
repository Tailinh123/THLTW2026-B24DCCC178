  ...
  return (
    ...
    >
      <Descriptions bordered column={2} size="small" style={{ marginTop: 12 }}>
        <Descriptions.Item label="Họ và tên" span={2}>{record.hoTen}</Descriptions.Item>
        ...
        <Descriptions.Item label="Câu lạc bộ" span={2}>{club?.tenCLB ?? record.clubId}</Descriptions.Item>
        <Descriptions.Item label="Ngày đăng ký">{moment(record.ngayDangKy).format('DD/MM/YYYY')}</Descriptions.Item>
        <Descriptions.Item label="Trạng thái">
          <Tag color={STATUS_COLOR[record.trangThai]}>{STATUS_LABEL[record.trangThai]}</Tag>
        </Descriptions.Item>
        ...
        {record.trangThai === 'Rejected' && (
          <Descriptions.Item label="Lý do từ chối" span={2}>
            <span style={{ color: '#ff4d4f' }}>{record.lyDoTuChoi || '—'}</span>
          </Descriptions.Item>
        )}
      </Descriptions>
    </Modal>
